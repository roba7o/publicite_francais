...
class FranceInfoArticleParser(BaseParser):
    ...
    def parse_article(self, soup: BeautifulSoup) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not content_div:
                self.logger.warning(
                    "Main content div ('c-body') not found for FranceInfo article."
                )
                return None
            ...
            if not full_text:
                self.logger.warning(
                    "No significant text extracted from FranceInfo article."
                )
                return None
            ...
            if self.debug:
                ...
                self.logger.info(
                    f"FranceInfo text stats: unique_words={stats['total_unique_words']}, "
    ...
    def _extract_paragraphs(self, content_div: BeautifulSoup) -> List[str]:
        ...
        for element in content_div.find_all(
            ["p", "h2", "li"]
        ):  # Added 'li' for list items often found in content
            if element.name == "h2":
