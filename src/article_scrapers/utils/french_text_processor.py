...
class FrenchTextProcessor:
    ...
    def __init__(self):
        ...
        self.french_contractions_prefixes = {
            "l'",
            "d'",
            "qu'",
            "m'",
            ...
            for prefix in self.french_contractions_prefixes:
                if processed_token.startswith(prefix):
                    ...
                    if base_word and base_word not in self.french_stopwords:
                        ...
                        processed_token = (
                            ""  # Mark for removal if base word is also a stopword
                        )
                    break  # Only apply one prefix removal

            if (
                not processed_token
            ):  # Remove if marked for removal (e.g., "l'on" where "on" is a stopword)
                continue

            # Apply length limits *after* contraction stripping
            if (
                len(processed_token) < self.min_word_length
                or len(processed_token) > self.max_word_length
            ):
                continue
