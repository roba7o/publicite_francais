...
def main() -> Optional[int]:
    ...
    try:
        ...
        logger.info(f"Starting pipeline with {len(source_configs)} sources", 
        ...
        if success_rate < 50.0:
            cli.warning(f"Low success rate ({success_rate:.1f}%) - check logs for issues")
            logger.warning("Pipeline completed with low success rate", 
                          extra_data={"success_rate": success_rate})

        cli.info("Check database:")
        cli.info('docker compose exec postgres psql -U news_user -d french_news -c "SELECT title, LENGTH(full_text), scraped_at FROM news_data_test.articles ORDER BY scraped_at DESC LIMIT 3;"')
