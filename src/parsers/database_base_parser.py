            ...
            try:
                ...
                if len(response.content) < 100:
                    self.logger.warning(
                        ...
                        extra_data={"url": url, "content_length": len(response.content)}
                    )
                    continue
    ...
    def get_test_sources_from_directory(self, source_name: str) -> List[Tuple[Optional[BeautifulSoup], str]]:
        ...
        if not source_dir.exists():
            self.logger.warning(
                ...
                extra_data={"source_name": source_name, "directory_path": str(source_dir)}
            )
            return []
    ...
    def to_database(self, article_data: ArticleData, url: str) -> bool:
        ...
        try:
            with self.db.get_session() as session:
                ...
                """), {"source_id": self.source_id, "url": url}).fetchone()
