...
class TestUrlCollectorImports:
    ...
    def test_slate_fr_url_collector_import(self):
        """Test SlateFrUrlCollector can be imported and instantiated."""
    ...
    def test_france_info_url_collector_import(self):
        """Test FranceInfoUrlCollector can be imported and instantiated."""
        from core.components.url_collectors.france_info_url_collector import FranceInfoUrlCollector
        
        collector = FranceInfoUrlCollector(debug=True)
        assert collector is not None
        assert hasattr(collector, 'get_article_urls')

    def test_tf1_info_url_collector_import(self):
        """Test TF1InfoUrlCollector can be imported and instantiated."""
        from core.components.url_collectors.tf1_info_url_collector import TF1InfoUrlCollector
        
        collector = TF1InfoUrlCollector(debug=True)
        assert collector is not None
        assert hasattr(collector, 'get_article_urls')

    def test_ladepeche_fr_url_collector_import(self):
        """Test LadepecheFrUrlCollector can be imported and instantiated."""
        from core.components.url_collectors.ladepeche_fr_url_collector import LadepecheFrUrlCollector
        
        collector = LadepecheFrUrlCollector(debug=True)
        assert collector is not None
        assert hasattr(collector, 'get_article_urls')
...
class TestUrlCollectorInterface:
    ...
    def test_all_collectors_have_required_methods(self):
        ...
        for collector in collectors:
            ...
            assert hasattr(collector, 'get_article_urls')
            assert callable(getattr(collector, 'get_article_urls'))
