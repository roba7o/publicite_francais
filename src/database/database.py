...
class DatabaseManager:
    ...
    def get_connection(self) -> Generator[psycopg2.extensions.connection, None, None]:
        ...
        try:
            ...
            if connection:
                connection.rollback()
            self.logger.error(
                "Database connection error", extra_data={"error": str(e)}, exc_info=True
            )
            raise
    ...
    def get_session(self) -> Generator[Session, None, None]:
        ...
        try:
            ...
            session.rollback()
            self.logger.error(
                "Database session error", extra_data={"error": str(e)}, exc_info=True
            )
            raise
        ...
        try:
            ...
            with self.get_session() as session:
                result = session.execute(
                    text("SELECT COUNT(*) FROM news_data.news_sources")
                )
                count = result.scalar()
    ...
    def health_check(self) -> dict:
        ...
        if not health_status["errors"]:
            ...
            health_status["status"] = (
                "degraded" if len(health_status["errors"]) == 1 else "unhealthy"
            )

        self._last_health_check = current_time

        self.logger.info("Database health check completed", extra_data=health_status)
