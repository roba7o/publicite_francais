...
class ConfigurableMockParser:
    ...
    def __init__(self, source_id: str = "mock-source-id", behavior: str = "success", debug: bool = False):
        """
    ...
    def parse_article(self, soup: BeautifulSoup) -> Optional[ArticleData]:
        ...
        config = self.content_configs.get(self.behavior, self.content_configs['success'])
...
def create_mock_parser(source_id: str = "mock-source-id", debug: bool = False) -> ConfigurableMockParser:
    """Create standard success mock parser."""
    return ConfigurableMockParser(source_id, behavior="success", debug=debug)


def create_failing_mock_parser(source_id: str = "mock-failing-id", debug: bool = False) -> ConfigurableMockParser:
    """Create failing mock parser."""
    return ConfigurableMockParser(source_id, behavior="failure", debug=debug)


def create_rich_content_mock_parser(source_id: str = "mock-rich-id", debug: bool = False) -> ConfigurableMockParser:
    """Create mock parser with rich French content."""
    return ConfigurableMockParser(source_id, behavior="rich_content", debug=debug)


def create_empty_mock_parser(source_id: str = "mock-empty-id", debug: bool = False) -> ConfigurableMockParser:
...
MockParser = lambda debug=False: create_mock_parser("mock-source-id", debug)
